...
Output Summary: All nine policy files read in the order above. Hard constraints carried into execution: no new dependency; no new `# noqa` / `# type: ignore` / `# pyright: ignore`; 500-line file cap on production code; no runtime temp files in tests; mandatory four-stage Python toolchain loop (black -> ruff -> pyright -> pytest with coverage); coverage thresholds line >= 85%, branch >= 75% uniform across T1-T4.
